    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Track platform revenue, vendor payouts, and transaction logs.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <div key={s.label} className="p-6 bg-card rounded-2xl border border-border shadow-sm">
            ...
            <p className="text-[10px] font-bold text-muted-foreground mt-2">{s.change}</p>
          </div>
        ))}
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-border bg-secondary/50">
                <th className="px-6 py-4 text-xs font-bold text-muted-foreground uppercase tracking-widest">Transaction</th>
                <th className="px-6 py-4 text-xs font-bold text-muted-foreground uppercase tracking-widest">Type</th>
                <th className="px-6 py-4 text-xs font-bold text-muted-foreground uppercase tracking-widest">Amount</th>
                <th className="px-6 py-4 text-xs font-bold text-muted-foreground uppercase tracking-widest">Method</th>
              ...
              {transactions.map((tx) => (
                <tr key={tx.id} className="hover:bg-secondary/50 transition-colors">
                  <td className="px-6 py-4">
                    <p className="text-sm font-bold text-foreground">{tx.id}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">{tx.user}</p>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    ...
                    <p className="text-[10px] text-muted-foreground mt-0.5 whitespace-nowrap">{tx.date}</p>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      {tx.method.includes('Visa') || tx.method.includes('Mastercard') ? <CreditCard className="w-4 h-4 text-muted-foreground" /> : <Banknote className="w-4 h-4 text-muted-foreground" />}
                      <span className="text-xs font-medium text-muted-foreground">{tx.method}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
