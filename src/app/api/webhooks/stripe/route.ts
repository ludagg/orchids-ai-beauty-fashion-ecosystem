// [Jules - Configured webhook secret verification, updated loggers, and mapped product schema issues]
...
export async function POST(req: NextRequest) {
  if (!process.env.STRIPE_WEBHOOK_SECRET) {
    ...
    return NextResponse.json({ error: "Webhook Error: STRIPE_WEBHOOK_SECRET is not configured." }, { status: 500 });
  }
  ...
  try {
    ...
    logger.error({ err }, `Webhook signature verification failed: ${err.message}`);
