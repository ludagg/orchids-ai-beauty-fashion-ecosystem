    ...
    if (searchTerms.length > 0) {
        for (const term of searchTerms) {
             conditions.push(or(
                 ...
                 ilike(products.mainCategory, `%${term}%`), ilike(products.subcategory, `%${term}%`)
             ));
        }
    }
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (allTerms.length > 0) {
            if (categories.length > 0) {
                 const catConditions = categories.map((c: string) => or(
                     ilike(products.mainCategory, `%${c}%`), ilike(products.subcategory, `%${c}%`),
        ...
        if (resultProducts.length === 0 && allTerms.length > 0) {
             const relaxedConditions: SQL[] = [eq(products.status, 'ACTIVE'), eq(products.visibility, 'PUBLIC')];
             const termConditions = allTerms.map((t: string) => or(
                 ilike(products.name, `%${t}%`),
                 ilike(products.description, `%${t}%`),
                 ilike(products.mainCategory, `%${t}%`), ilike(products.subcategory, `%${t}%`)
             ));
