      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {["Weekly", "Monthly"].map((t) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
                    timeRange === t ? "bg-card text-foreground shadow-sm" : "text-muted-foreground/50 hover:text-muted-foreground"
                  }`}
                >
                  {t}
                </button>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data}>
                ...
                <Tooltip
                  ...
                  contentStyle={{ borderRadius: '16px', backgroundColor: 'var(--card)', borderColor: 'var(--border)', color: 'var(--foreground)' }}
                  itemStyle={{ color: 'var(--foreground)' }}
                />
                <Bar dataKey="revenue" fill="var(--foreground)" radius={[6, 6, 0, 0]} barSize={40} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-[40px] shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-2 no-scrollbar">
            {transactions.map((tx) => (
              <div key={tx.id} className="p-6 hover:bg-muted/30 transition-all rounded-3xl border border-transparent hover:border-muted group">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] font-bold text-muted-foreground/50 uppercase tracking-widest">{tx.id}</span>
                  <span className={`text-[8px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${
                    tx.status === 'Success' ? 'bg-emerald-500/10 text-emerald-600' : 'bg-amber-500/10 text-amber-600'
                  }`}>
                    {tx.status}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] text-muted-foreground/50 font-bold mt-0.5">{tx.time}</p>
                  </div>
                </div>
              </div>
            ))}
