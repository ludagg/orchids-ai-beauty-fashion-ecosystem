...
export default function SalonsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-[1400px] mx-auto w-full">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-2 px-3 sm:px-4 py-2.5 rounded-xl border text-sm font-medium hover:bg-secondary transition-all focus-visible:ring-2 focus-visible:ring-primary ${
                sortBy === "rating"
      ...
      {/* Categories Scroll */}
      <section className="flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar" aria-label="Salon categories">
        {categories.map((category) => (
          <button
            key={category}
            onClick={() => setSelectedCategory(category)}
            aria-pressed={selectedCategory === category}
            className={`px-5 py-2.5 rounded-full text-sm font-medium whitespace-nowrap transition-all focus-visible:ring-2 focus-visible:ring-primary ${
      ...
      <div className="flex sm:hidden items-center gap-2 p-1 bg-secondary rounded-xl w-fit" aria-label="View switch">
        <button
          ...
          <div className="hidden sm:grid sm:grid-cols-[1fr_360px] lg:grid-cols-[1fr_420px] gap-6 items-start">
            <div className="grid grid-cols-1 gap-6">
              ...
              {filteredSalons.length === 0 && (
                <Empty className="py-12 border border-dashed border-border rounded-3xl">
                  <EmptyHeader>
                    ...
                    <EmptyDescription>
                      We couldn&apos;t find any salons matching your search or filters. Try adjusting your search query or selecting a different category.
                    </EmptyDescription>
                  </EmptyHeader>
                </Empty>
              )}
      ...
      >
        <div className="relative z-10 max-w-lg">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button
              type="button"
              className="px-8 py-4 rounded-2xl bg-card text-blue-600 font-bold hover:bg-card/90 transition-all flex items-center justify-center gap-2 focus-visible:ring-2 focus-visible:ring-white"
            >
              Claim Offer <ChevronRight className="w-4 h-4" aria-hidden="true" />
            </button>
            <button
              type="button"
              className="px-8 py-4 rounded-2xl bg-card/10 backdrop-blur-md border border-white/20 text-white font-bold hover:bg-card/20 transition-all focus-visible:ring-2 focus-visible:ring-white"
            >
              Learn More
            </button>
          </div>
  ...
  return (
    ...
    >
      <Link
        ...
        className="flex flex-col sm:flex-row gap-5 p-4 rounded-3xl bg-card border border-border hover:shadow-xl hover:shadow-foreground/5 transition-all group overflow-hidden focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
      >
        ...
        <div className="flex-1 flex flex-col justify-between py-1">
          ...
          <div className="flex items-center justify-between mt-6 sm:mt-0 pt-4 border-t border-border sm:border-0">
            ...
            <span className="px-5 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-bold group-hover:bg-blue-600 transition-all flex items-center gap-2 shadow-lg shadow-foreground/5">
