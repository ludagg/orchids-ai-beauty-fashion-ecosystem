...
export default function ConversationsPage() {
  ...
  return (
    <div className="h-[calc(100vh-64px)] lg:h-[calc(100vh-64px)] bg-background flex overflow-hidden">
        ...
        <div className="p-6 border-b border-border space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 rounded-2xl bg-muted border-transparent focus:bg-card focus:border-border transition-all outline-none text-sm text-foreground"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          {chats.map((chat) => (
            <button
              ...
              className={`w-full p-4 flex gap-4 transition-all border-b border-muted/50 hover:bg-muted/30 ${
                selectedChat?.id === chat.id ? "bg-muted/50 border-l-4 border-l-foreground" : ""
              }`}
            >
              ...
              <div className="flex-1 min-w-0 text-left">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[10px] text-muted-foreground/60 font-bold">{chat.time}</span>
                </div>
      ...
      {/* Chat Window */}
      <main className={`flex-1 flex flex-col bg-background/50 ${!selectedChat && 'hidden lg:flex'}`}>
        {selectedChat ? (
          <>
            {/* Chat Header */}
              ...
              {messages.map((msg) => (
                ...
                >
                  <div className={`max-w-[70%] space-y-1 ${msg.sender === 'me' ? 'items-end' : 'items-start'}`}>
                    ...
                    <div className={`flex items-center gap-1.5 text-[10px] font-bold text-muted-foreground/60 ${msg.sender === 'me' ? 'flex-row-reverse' : ''}`}>
                      {msg.time}
                      {msg.sender === 'me' && (
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full pl-4 pr-12 py-4 rounded-2xl bg-muted border-transparent focus:bg-card focus:border-border transition-all outline-none text-sm text-foreground"
                  />
                ...
                <button
                  ...
                  className="w-12 h-12 rounded-2xl bg-primary text-primary-foreground flex items-center justify-center hover:opacity-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-xl shadow-black/10 dark:shadow-white/5"
                >
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center space-y-6">
            ...
            <button className="px-8 py-3.5 rounded-2xl bg-primary text-primary-foreground font-bold hover:opacity-90 transition-all shadow-xl shadow-black/10 dark:shadow-white/5">
              New Message
            </button>
          </div>
        )}
        ...
        <aside className="hidden xl:flex w-80 border-l border-border flex-col bg-card">
          <div className="p-8 text-center space-y-6">
            ...
            <div className="flex justify-center gap-3">
              <button className="p-3 rounded-2xl border border-border hover:bg-muted transition-all text-foreground">
          ...
          <div className="flex-1 overflow-y-auto p-8 border-t border-border space-y-8">
            <section>
              <h4 className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-[0.2em] mb-4">Shared Media</h4>
            ...
            <section>
              <h4 className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-[0.2em] mb-4">Settings</h4>
              <div className="space-y-2">
                ...
                <button className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-muted text-muted-foreground transition-all">
                  ...
                  <div className="w-8 h-4 bg-muted-foreground/20 rounded-full relative">
                    <div className="absolute left-1 top-1 w-2 h-2 bg-primary/20 rounded-full" />
                  </div>
                </button>
              </div>
            </section>
