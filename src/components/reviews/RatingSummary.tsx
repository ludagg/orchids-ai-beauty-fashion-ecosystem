...
/* [Jules - Enhance accessibility with role="img" and descriptive aria-labels for screen reader clarity] */
            ...
            return (
              <div key={star} className="flex items-center gap-3 text-xs font-medium">
                ...
                <Progress
                  ...
                  aria-label={`${star} star${star > 1 ? "s" : ""}: ${Math.round(percentage)}%`}
                />
                <span className="w-8 text-right text-muted-foreground" aria-label={`${count} reviews`}>{count}</span>
              </div>
            );
          })}
        </div>
