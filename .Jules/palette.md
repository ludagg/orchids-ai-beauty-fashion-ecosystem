...
## 2026-07-18 - Keyboard Focusable File Inputs
**Learning:** Standardizing file upload labels with `focus-within:ring-2` is completely ineffective if the inner `<input type="file">` is visually hidden with `display: none` or `className="hidden"`, as it removes it from keyboard focus flow.
**Action:** Use the `sr-only` utility class on hidden inputs so screen readers and keyboard tab navigation can focus them, successfully triggering the parent's `focus-within:ring-2` ring overlay styles.
