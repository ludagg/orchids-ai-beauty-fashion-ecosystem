...
## 2026-04-14 - Mobile Navigation Accessibility
**Learning:** For bottom navigation elements on mobile screens, utilizing explicit aria-label attributes and matching the active tab with dynamic aria-current="page" state indicators is crucial for screen readers. Furthermore, adding focus-visible keyboard outlines and hiding decorative elements inside the interactive links using aria-hidden="true" prevents redundant text readouts and ensures the elements are fully keyboard navigatable.
**Action:** Standardize bottom nav components to include explicit aria-label, dynamic aria-current="page" state indicators, and focus-visible keyboard ring outlines while hiding decorative inner components with aria-hidden="true".
