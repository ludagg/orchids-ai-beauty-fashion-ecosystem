...
## 2026-06-14 - Accessible Custom Progress Indicators
**Learning:** Custom progress bars (like those in story viewers) are invisible to assistive technologies unless explicitly marked with `role="progressbar"`. Providing `aria-valuenow`, `aria-valuemin`, and `aria-valuemax` ensures screen readers can announce the current state of the interaction.
**Action:** Always add semantic ARIA roles and value attributes to custom-built progress or loading indicators.
