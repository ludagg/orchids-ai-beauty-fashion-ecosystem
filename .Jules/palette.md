...
## 2025-05-14 - Accessible Hero Carousels and Guest Access

**Learning:** Hero carousels often lack semantic roles, making them confusing for screen readers. Using `role="region"` and `aria-roledescription="carousel"` provides necessary context. However, `aria-live="polite"` should be avoided on auto-playing carousels as it causes constant, disruptive announcements. Additionally, for "Discovery-first" apps, the main dashboard should be public to reduce friction and improve initial engagement.
**Action:** Always wrap carousels in a semantic region with an accessible label. Avoid `aria-live` on auto-play sections. Ensure guest-facing routes like `/app` (Discover) are explicitly added to public path lists in middleware to prevent unnecessary login redirects.
