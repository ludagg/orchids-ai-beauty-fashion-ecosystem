...
## 2026-04-14 - Accessible Notification Timestamps
**Learning:** In interactive lists like notifications, where items are semantic buttons with `aria-label`, any informative child content (like relative timestamps) is lost to screen readers if not explicitly included in the label.
**Action:** Always include relative timestamps and status (e.g., 'Unread') in the `aria-label` of notification buttons: `aria-label="${isRead ? '' : 'Unread: '}${title}. ${message}. ${timeAgo}"`.
