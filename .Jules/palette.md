...
## 2026-04-11 - Valid HTML Nesting for Complex Cards
**Learning:** Nesting interactive elements like `<button>` inside a `<Link>` (anchor) is invalid HTML and causes issues with screen readers and event propagation.
**Action:** Implement secondary actions as absolute-positioned siblings to the primary link within a shared relative container. Use `pointer-events-none` on informational overlays and higher z-index on specific interactive buttons to ensure accessibility and correct click behavior.
